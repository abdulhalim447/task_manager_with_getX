...
class _SignInScreenState extends State<SignInScreen> {
  ...
  Future<void> initiateSignIn() async {
    bool status = await Get.find<AuthViewModelClass>().signInUser(
        email: _emailTEController.text.trim(),
        password: _passwordTEController.text,
        userViewModel: Get.find<UserViewModelClass>());
    if (mounted && status) {
      Get.find<TaskViewModelClass>().resetTaskData();
      Get.offAndToNamed(AppRoutes.dashboardScreen);
      Get.find<AuthViewModelClass>().setPasswordObscure = true;
      return;
    }
    if (mounted) {
      Failure failure = Get.find<AuthViewModelClass>().response as Failure;
