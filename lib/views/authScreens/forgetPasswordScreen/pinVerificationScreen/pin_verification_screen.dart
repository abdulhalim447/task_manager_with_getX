...
class _PinVerificationScreenState extends State<PinVerificationScreen> {
  ...
  void startCountDownTimer() async {
    Get.find<CountdownTimerViewModelClass>().resetCountDown();
    timer = Timer.periodic(const Duration(seconds: 1), (countdown) {
      if (countdown.tick > 60) {
        countdown.cancel();
        return;
      }
      Get.find<CountdownTimerViewModelClass>().updateCountdown();
    });
  }
  ...
  Future<void> initiatePinVerification() async {
    ...
    bool status = await Get.find<AuthViewModelClass>().verifyOTP(otp);
    if (status && mounted) {
      timer.cancel();
