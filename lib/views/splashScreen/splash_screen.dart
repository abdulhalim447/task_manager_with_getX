...
class _SplashScreenState extends State<SplashScreen> {
  ...
  void initState() {
    super.initState();
    Get.find<ConnectivityCheckerClass>().initConnectivityChecker();
    checkToken();
  }
  ...
  Future<void> checkToken() async {
    try {
      ...
      if (mounted) {
        bool status = await Get.find<AuthViewModelClass>().authenticateToken(token);
        if (status && mounted) {
          await Get.find<UserViewModelClass>().loadUserData(preferences);
